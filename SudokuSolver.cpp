	...
	: mGrid(nullptr), mGenModifier(false), mNumToIgnore({ 0, 0, 0 })
{
 ...
 void Solver::setElement(const int row, const int col, const int num)
{
	mGrid[row * 9 + col] = num;
}

int Solver::getElement(const int row, const int col) const
{
	return mGrid[row * 9 + col];
}

bool Solver::checkValid(const int inputRow, const int inputCol, const int num) const
{
...
void Solver::setGenModifier(const bool input)
{
	mGenModifier = input;
}

void Solver::setGrid(int* grid, const Ignore& ignoreInput)
{
