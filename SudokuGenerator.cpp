...
void Generator::setElement(const int row, const int col, const int num)
{
	mGrid[row * 9 + col] = num;
}

int Generator::getElement(const int row, const int col) const
{
	return mGrid[row * 9 + col];
}

void Generator::swapNumbers(const int index1, const int index2)
{
...
void Generator::swapRows(const int row1, const int row2)
{
...
void Generator::swapCols(const int col1, const int col2)
{
...
void Generator::swapRowBlocks(const int rowBlock1, const int rowBlock2)
{
...
void Generator::swapColBlocks(const int colBlock1, const int colBlock2)
{
...
void Generator::fillNextRow(const int previousRow, const int nextRow, const int shifts)
{
...
void Generator::copyGrid(int* grid) const
{
	for (int i = 0; i < 81; i++)
	{
		grid[i] = mGrid[i];
...
void Generator::createCompletedSudoku()
{
...
void Generator::generate(int* grid, int* solutionGrid, int lvl)
{
	...
	// Elements to remove
	if (lvl == 1){
        given = randRange(35, 38);
    }

    if (lvl == 2){
        given = randRange(31, 34);
    }

    if (lvl == 3){
        given = randRange(27, 30);
    }

    int toRemove = 81 - given;
	while (toRemove)
	{
